...
label staroslav:
    centered """
    Для сдачи экзамена по старославянскому языку тебе предстоит пройти игру "Мемо".
    Заинтриговали? Вот правила игры: Игрок переворачивает по две карточки таким образом,
    если картинки на карточках одинаковые, то игрок забирает их.
    Цель: собрать как можно больше парных карточек.
    Игра продолжается до тех пор, пока игрок не соберет все 10 пар карточек ИЛИ до конца времени.
    За каждую собранную пару игрок получает 1 балл.
    Максимум: 10 баллов.
    """
    nvl clear
